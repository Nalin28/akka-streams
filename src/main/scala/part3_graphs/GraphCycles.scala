...
object GraphCycles extends App{
  ...
  - since initially there is nothing in the preferred port the mergeShape takes input from the input port
  - after that when its processed and fed back to the preferred port it keeps incrementing from the preferred port indefinitely
   */
  val actualAccelerator = GraphDSL.create() { implicit builder =>
    import GraphDSL.Implicits._
    val sourceShape = builder.add(Source(1 to 100))
    val mergeShape = builder.add(MergePreferred[Int](1)) // because it has one additional port and a preferred port
    val incrementerShape = builder.add(Flow[Int].map{x =>
      println(s"Accelerating $x")
      x + 1
    })
  ...
  /*
  Solution 2: buffer (to overcome the buffer overflow strategy so that the packets are actually dropped)
   */

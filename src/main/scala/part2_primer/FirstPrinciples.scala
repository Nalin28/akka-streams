...
object FirstPrinciples extends App{
  ...
  implicit val materializer = ActorMaterializer() // It allows the running of akka streams
  ...
  val infiniteSource = Source(Stream(1)) // do not confuse an Akka stream with a "collection" Stream

  import scala.concurrent.ExecutionContext.Implicits.global
  val futureSource = Source.fromFuture(Future(42)) // sources emit asynchronously -> so as soon as the future is complete it will emit it
  ...
  val takeFlow = Flow[Int].take(5) // turns a stream to a finite stream
  // drop, filter
  // NOT have flatMap
  ...
  nameSource.filter(_.length > 5).take(2).runForeach(println) // same but short

...
object OperatorFusion extends App{
  ...
  // this runs on the SAME ACTOR -> fusion
  ...
  // simpleSource.via(complexFlow).via(complexFlow2).to(simpleSink).run()// 2 sec between each operation!
  // so it would be better to run them on different actors (different cores = parallel)
...
//    .run()
  ...
  // ordering guarantees - relative ordering 1,2,3 will be kept
  Source(1 to 3)

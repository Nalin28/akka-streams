...
object AdvancedBackpressure extends App{
  ...
  val events = List(
    ...
    PagerEvent("Number of HTTP 500 spike", new Date),
  ...
  val aggregateNotificationFlow = Flow[PagerEvent]
    // acts like fold but it emits elements when required by downstream
  ...
  val extrapolator = Flow[Int].extrapolate(element => Iterator.from(element)) // will fill in more dummy elements
  val repeater = Flow[Int].extrapolate(element => Iterator.continually(element)) // repeats the element

  slowCounter.via(extrapolator).to(hungrySink).run()

  val expander = Flow[Int].expand(element => Iterator.from(element))

  // difference btw expander and extrapolator is that extrapolator creates Iterator when there is demand expander creates The Iterator at all times
}

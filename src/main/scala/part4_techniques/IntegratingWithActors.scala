...
object IntegratingWithActors extends App{
  ...
  //val actorBasedFlow = Flow[Int].ask[Int](parallelism = 4)(simpleActor).to(Sink.ignore).run()
  // parallelism = how many messages can be there in the actor mailbox before the actor starts back pressuring if you send too many messages
  // or
  ...
  class DestinationActor extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      ...
      case message =>
        ...
        sender() ! StreamAck // if this is not given then backpressure will be applied
    }
  }
  ...
  ) // all these are called "life cycle" messages

  Source(1 to 10).to(actorPoweredSink).run()

  // Sink.actorRef() not recommended, unable to apply backpressure
